...
aliases: [MechanicalDrivePort, MechanicalDriveState, MechanicalRequirement, Primitive Combustion Engine, AlcoholicDebug]
sources:
...
summary: >-
  Loader-independent rotary port. Debug RPM NBT is compile-stripped from published jars.
provenance:
  extracted: 0.86
  inferred: 0.12
  ambiguous: 0.02
...
`minecraft-common` `MechanicalDrives` samples adjacent sources the same way `HeatSources` samples heat. A native block entity that implements `MechanicalDrivePort` and `isSource()` is read directly. Optional mods register a `MechanicalDrives.Probe`. Future Crossroads work is a new probe, not an edit to the Malt Mill.
...
The [[native-executor-invariant|Malt Mill]] reads adjacent drive state and stalls when the requirement is unmet. The industrial press can run from an engine against its kinetic port; the port is a `MechanicalDrivePort` relay, not a Create-only socket.
