...
`BuiltinCraftMachines` registers malt house, mill, mash tun, brewing kettle, and vat. Casing is `alcoholic:craft_casing`. They execute the same generic MALT / MILL / MASH / BOIL / FERMENT types as [[grain-processing]], not a second recipe system.

Size is variable. Mega-mesh overlays only the art size; other legal sizes keep the 9-slice hull ([[formed-multiblock-visual]]). Ports stay on real blocks at the faces the recipe requires, independent of mega-mesh size.

A formed craft malt house that is larger than the art board used to draw a broken overlay; the hull path is the fallback for those sizes. ^[inferred]
