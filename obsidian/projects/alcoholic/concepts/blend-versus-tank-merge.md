...
A datapack names accepted inputs, optional minimum fractions, and an output liquid (for example a cuvée). Shipped wine blend is young+young of the same colour → young; it does not skip the barrel. Addons declare aggregator semantics on the property, not in the tank. See [[playability-audit]].
