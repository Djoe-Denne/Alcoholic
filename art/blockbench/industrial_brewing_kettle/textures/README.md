...
Mega-mesh = overlay BER `models/block/formed/industrial_brewing_kettle.json` at art size 5×6×5 only.
...
- **Id / type** : `industrial_brewing_kettle` — **vue d’ensemble formée**. Ce n’est **pas** un bloc posable et **pas** `brewing_kettle` (artisanal). Overlay BER à 5×6×5 ; sinon hull 9-slice + `industrial_brewing_kettle_controller`.
...
| BER formé | `FormedMultiblockRenderer` — hull 9-slice si `formed` ; mega-mesh seulement à 5×6×5 |
